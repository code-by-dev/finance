// cspell:words Inngest GenerativeAI lte
...
export const checkBudgetAlerts = inngest.createFunction(
  ...
  async ({ step }) => {
    const budgets = await step.run("fetch-budgets", async () => {
      await connectDB();
      return await Budget.find().populate("userId").lean();
    });
    ...
    for (const budget of budgets) {
      ...
      const budgetId = budget._id?.toString?.() ?? budget.id;
      await step.run(`check-budget-${budgetId}`, async () => {
        await connectDB();
        ...
        const result = await Transaction.aggregate([
          {
            $match: {
              userId: budget.userId?._id ?? budget.userId,
